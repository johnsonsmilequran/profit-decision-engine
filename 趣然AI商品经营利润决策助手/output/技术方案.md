...
> OA 实现同步（2026-08-04）：`API-F06-05` 已使用服务端 `OA_MESSAGE_URL` + Bearer `OA_TOKEN` 调用公司 OA 适配入口，出站 JSON 为独立白名单 DTO，仅含 `recipient_actor_ref/template_code/spu_id/action/operator/feedback_request/task_reference`；`spu_id` 是当前商品链接/SPU 标识，`task_reference` 是稳定产品内任务引用，成功响应须返回非空 `message_id`。手动发送、失败补发、发送尝试次数、受控错误码和回执均持久化；发送成功不改变库存协同状态。Worker 按 Asia/Shanghai 自然日创建清仓催办，以任务、日期、接收人和模板唯一约束防重复，确认、非清仓改判或终止后停止新提醒；接收人映射缺失时保留失败记录而不伪造发送。服务时钟在生产默认读取系统时间，测试注入固定时间；真实 PostgreSQL 并发验收覆盖两个连续自然日、失败补发以及确认/改判/终止后的停催。
