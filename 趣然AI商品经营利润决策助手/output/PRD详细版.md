...
> **F09 MVP 完成定义**：F09-SCOPE-01—06 全部交付；核心闭环从无有效唯一角色的钉钉用户登录开始，经过待处理申请、Admin 核验身份和审批依据、配置唯一正确角色并撤销旧业务会话，最终由用户重新登录进入正确工作台。同时，观察期内经确认的错误业务授权事件为 0，应审计的高权限状态变更审计覆盖率为 100%，每次恢复后仍可用的旧 Admin 会话为 0。首次启用的 10 分钟结果只有在 §0.2 全部门禁满足后才有效。来源：用户确认的 `proposal-v1.md` MVP 完成定义，依据等级 B。
...
- **生产门禁**：§0.2 的十项检查；不同门禁只阻断其明确层级，不实现成一个通用全站关闭开关。
...
| :--- | :--- | :--- | :--- | :--- |
| 首次初始化授权闭环时间 | 空角色环境无法通过产品完成，时间不可控 | ≤10 分钟，待真实首用验证 | 首次部署；仅在 §0.2 全部门禁满足后有效 | §8.3 固定七个服务端里程碑；用户确认的 REQ-F09-07；B |
...
| 完整授权周期 | 未记录 | 记录真实分布，不虚构 SLA | 上线后 30 天首次复盘 | 每次新增、变更、停用或恢复使用独立授权周期；待处理身份产生至下一次正确角色业务会话进入工作台，包含外部审批等待；缺节点时显示“无法形成完整周期”，不填 0；B |
...
- 首次初始化授权闭环以里程碑 1 为起点、里程碑 7 为终点，只有 §0.2 全部门禁已满足时，`≤10 分钟` 结果才有效。
...
| 钉钉统一身份认证 | 为运营和运营主管提供独立身份并承载双角色映射 | 公司钉钉身份由运维/系统管理员管理 | 事业部负责人审批角色变更；运维/系统管理员按审批结果维护映射及接入配置 | 钉钉身份体系与本产品服务端认证配置；产品无业务侧角色后台 | 玩具事业部运营、运营主管 | MVP 只验收本机普通浏览器点击钉钉 OAuth 登录；扫码登录和钉钉客户端/工作台入口不在本次范围 | 认证失败、登录态失效或无映射时默认拒绝，不为采购/仓库等相关人员建立产品角色 | REQ-F07-01～REQ-F07-03；AC-F07-01～AC-F07-06；NFR-002、NFR-005 |
...
当 §0.2 的 F09 第一版门禁全部满足并开始首次启用闭环时，系统应记录 §8.3 固定的七个服务端里程碑；从“唯一 Admin 初始化开始”至“目标身份首次以正确业务会话访问对应工作台”应不超过 10 分钟。任一里程碑缺失时不得显示为已达成或以 0 代替。
...
当正常凭据和保存型恢复码均不可用，且运维依据独立恢复审批提交当前环境有效的部署恢复口令，并完成新密码及新 TOTP 实码验证时，只有新凭据和恢复码生效、凭据代次更新、全部旧 Admin 会话撤销、部署恢复口令消费、恢复审计和通知任务持久化同时成功，系统才显示“恢复已安全完成”；内部任一步失败时旧凭据、旧会话和恢复口令状态均不得部分切换。外部通知投递失败不回滚已经安全完成的恢复，系统应持续显示“安全通知处置失败待处理”并告警；只有投递成功，或公司认可的独立审批人、恢复执行人和可追溯补偿证据齐备时，才显示“安全通知处置已闭环”。
...
| NFR-009 授权正确性与撤销及时性 | 非法授权被阻止；经确认的错人、错角色事件为零；角色变化或恢复后旧会话不可用且可见为已撤销 | AC-F09-06、AC-F09-09—13、AC-F09-15 |
...
| 身份认证与角色映射 | 已确认 | 使用钉钉统一身份认证；事业部负责人审批运营、运营主管角色变更，运维/系统管理员按审批结果配置；外部相关人员不建立产品角色 |
