...
这是运营和运营主管在办公时间长期使用的数据密集型工具；采购、仓库等相关人员只接收 OA 协同消息。
...
- 当前稳定任务 ID、当前动作、当前状态和当前生命周期时间常驻展示；最近前序建议快照放入每条 SPU 独立的披露控件，默认折叠。展开后必须同时呈现该前序批次自己的主动作、关键依据、库存语境、经营状态，以及来源批次、产生时间和执行时间，不能只显示关系元数据。
- 折叠态只显示“最近前序待办”和“展开查看上次建议快照”，不透出历史字段；展开内容采用“身份与时间条 + 主动作 / 关键依据 / 库存语境 / 经营状态”四区结构，并明确标注“前序冻结值”，禁止用当前周数据回填。使用原生 `details/summary` 或等价的可访问披露模式，支持鼠标点击与键盘 Enter/Space，展开动作不得触发行跳转。
...
| SPU 515 闭环状态 | 主管已通过；责任运营陈雨欣待执行清仓并协调禁补；清仓完成时间待提交；OA 每日催办开启；AI 已生成 |
| SPU 515 跨周待办 | 稳定任务 `TASK-TOY-515`；首次产生 `2026-06-29 09:12`；本周关联 `2026-07-06 09:18`；当前经营状态待执行；当前执行时间 `—` |
| SPU 515 最近前序建议快照（原型演示值） | 来源批次 `BATCH-20260531-01`；前序主动作清仓；前序关键依据为净销售额 `78,960 元`、经营准利润率 `-10.6% < 5%`、品退率未校验；前序库存语境为仓内 `2,850 件`、在途 `400 件`、近 14 天销量 `98 件`、日均 `7.0 件`、可售 `464.3 天`、禁止补货；前序经营状态待执行；产生时间 `2026-06-29 09:12`；执行时间 `—`。这些值只属于前序冻结快照，不代表当前批次。 |
