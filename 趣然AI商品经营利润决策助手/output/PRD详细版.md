...
| 钉钉统一身份认证与角色映射 | 已选定，接入可用性待实施验证 | 事业部负责人审批；运维/系统管理员配置 | 用户确认以钉钉为统一身份入口，只映射运营和运营主管两个直接业务角色 | 认证失败或无有效映射时拒绝进入业务页；不得降级为共享账号、账号密码或仅前端隐藏 |
| 公司 OA 通知通道 | 已确认需要，接口可用性待实施验证 | 运维/系统管理员配置；运营使用 | 用于通知采购、仓库等相关人员和每日催办责任运营确认清仓完成时间 | OA 失败时保留“待通知/通知失败”状态并由运营人工补发；不得伪造送达或业务确认 |
...
本产品只有运营和运营主管两个直接业务角色。建议由运营主管一次审核；若主管不采纳固定规则动作，可在任一动作开始执行前建立人工改判版本。改判只改变“生效动作”，原规则结论与冻结证据永久保留；清仓改为加投时必须同步重新确认库存动作，不得沿用禁补。每周新批次仍产生独立规则快照，但同一 SPU 同动作直接关联已有任务并保留其审核/执行状态；动作变化才追加新版本等待主管确认。当前待办必须显示最近一条前序经营待办记录的来源批次、动作、状态、产生时间和执行时间。建议通过后，该 SPU 责任运营统一执行经营动作、通过公司 OA 协调采购/仓库等相关人员并核验录入协同状态，外部相关人员不直接登录或修改本产品。
...
| 钉钉统一身份认证 | 为运营和运营主管提供独立身份并承载双角色映射 | 公司钉钉身份由运维/系统管理员管理 | 事业部负责人审批角色变更；运维/系统管理员按审批结果维护映射及接入配置 | 钉钉身份体系与本产品服务端认证配置；产品无业务侧角色后台 | 玩具事业部运营、运营主管 | 按审批结果维护；实施阶段验证连通性 | 认证失败、登录态失效或无映射时默认拒绝，不为采购/仓库等相关人员建立产品角色 | REQ-F07-01～REQ-F07-03；AC-F07-01～AC-F07-06；NFR-002、NFR-005 |
| 公司 OA 通知通道 | 协调外部相关人员并每日催办未确认清仓完成时间的责任运营 | OA 接口凭据由运维/系统管理员管理 | 运维/系统管理员配置接口与消息模板，责任运营发起或人工补发 | 服务端通知适配层；普通用户无凭据入口 | 玩具事业部协同消息 | 实施阶段验证连通性；按自然日幂等 | 失败保留“通知失败”和人工补发入口，不伪造已送达或业务已确认 | REQ-F06-02、REQ-F06-05、REQ-F07-02；AC-F06-04、AC-F06-10～12、AC-F07-03～04 |
...
| 公司 OA 通知能力 | 已确认需要，接入可用性待实施验证 | 运维/系统管理员 | 验证消息模板、接收人映射、发送回执与按自然日幂等键 | 失败显示待补发并由责任运营人工 OA；不改变业务确认状态 | REQ-F06-02、REQ-F06-05、REQ-F07-02；AC-F06-04、AC-F06-10～12、AC-F07-03～04 |
...
当责任运营需要推进补货或禁止补货协同动作时，系统应允许责任运营通过 OA 通知相关人员，并只由责任运营保存协同动作状态、时间、备注和经其核验的结果；OA 发送成功不得自动等同于业务状态已确认。
...
当当前生效动作仍为清仓且最终清仓完成时间尚未获得运营主管确认时，系统应按 `Asia/Shanghai` 自然日向该 SPU 责任运营发送至多一次 OA 催办，并记录消息日期、接收人、发送结果和关联任务；动作改判、终止或主管确认完成后立即停止，发送失败不得伪造已送达且应保留人工补发入口。
...
当系统向外部相关人员发送 OA 协同通知时，消息只应包含完成协同所需的 SPU 标识/商品链接、协同动作、责任运营、要求反馈内容和产品内任务引用，不包含完整利润、推广、品退、售后、规则阈值或主管审核详情。
...
| 用户关于身份认证与角色收敛的确认 | 钉钉是统一身份入口；直接业务角色只有运营和运营主管；其他相关人员由运营通过 OA 协同并闭环 | B |
