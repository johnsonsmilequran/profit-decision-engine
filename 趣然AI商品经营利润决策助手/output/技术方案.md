...
> 钉钉通知实现约束（2026-08-04）：用户已确认原规格中的 OA 即钉钉。`API-F06-05` 使用服务端 `DINGTALK_CLIENT_ID`、`DINGTALK_CLIENT_SECRET` 和 `DINGTALK_ROBOT_CODE` 获取应用 Access Token，并调用企业内部机器人单聊 `POST /v1.0/robot/oToMessages/batchSend`；收件人是公司钉钉 User ID，不使用 Union ID、手机号 @ 或群自定义机器人 Webhook。消息正文白名单仅含 SPU/商品链接、协同动作、责任运营、反馈要求和稳定产品内任务引用，协议外层仅含机器人编码、User ID、消息类型和消息参数。手动发送、失败补发、发送尝试次数、受控错误码和真实接口请求标识（如钉钉返回）均持久化；接口受理只记“钉钉已受理”，不得显示为已送达、已读或业务已确认。副作用消息请求不自动重试，以免重复发送；业务幂等仍由产品通知唯一键保障。Worker 按 Asia/Shanghai 自然日创建清仓催办，确认、非清仓改判或终止后停止新提醒；责任运营 User ID 映射缺失时保留失败记录。完整组织同步不属于本次产品范围，只复用现有映射或由运维配置公司 User ID。
