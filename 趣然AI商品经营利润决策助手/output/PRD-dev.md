...
| FR-7 / F07 | 权限隔离 | 钉钉统一认证、IT 角色映射、角色视图与操作权限 | 无认证/无映射默认拒绝；采购不见完整利润、推广、品退和售后 |
...
三类直接用户只通过钉钉独立登录，运营、运营主管、采购计划角色映射由 IT 维护。产品不提供账号密码、注册、找回密码或业务侧角色管理后台；认证失败、登录态失效或无映射时默认拒绝且不返回经营数据。
...
- 钉钉认证成功且有 IT 角色映射时建立对应权限登录态；认证失败、登录态失效或无映射时默认拒绝。
...
| 4 | F07 权限 | 钉钉接入与 IT 角色映射 | 三角色读取与操作均由服务端限制；无认证/无映射默认拒绝 |
...
| 钉钉统一身份 + IT 角色映射 | 共享账号、产品账号密码或业务侧权限后台 | 统一身份可追溯到个人，且避免采购权限只靠前端隐藏 | 依赖钉钉接入和 IT 维护映射的可用性 |
