...
function DetailedActionTable({ items, historyMode, expanded, setExpanded }: { items: ActionItem[]; historyMode: boolean; expanded: Record<string, boolean>; setExpanded: (value: Record<string, boolean>) => void }) {
  return <div className="table-scroll"><table className="action-list-table"><thead><tr><th>SPU 对象</th><th>发现问题与关键依据</th><th>双轨动作与执行进度</th><th>审核 / AI / 数据限制</th><th>责任人与时间</th><th /></tr></thead><tbody>{items.flatMap(item => {
    const rows = [<tr key={item.link_id}>
      ...
      <td><Track label="经营动作" action={item.effective_business_action} state={item.business_state} /><Track label="库存/钉钉协同" action={item.effective_inventory_action} state={item.inventory_state} /></td>
      <td><StatusLine label="审核" value={reviewText(item.review_status)} tone={item.review_status === 'approved' ? 'ready' : item.review_status === 'rejected' ? 'failed' : 'pending'} /><StatusLine label="AI" value={aiText(item.latest_ai_status)} tone={item.latest_ai_status === 'generated' ? 'ready' : item.latest_ai_status === 'generating' ? 'pending' : 'failed'} /><StatusLine label="钉钉" value={notificationText(item.latest_notification_status, item.latest_notification_error_code)} tone={item.latest_notification_status === 'sent' ? 'ready' : item.latest_notification_status === 'pending' ? 'pending' : 'failed'} />{item.latest_clearance_status ? <StatusLine label="完成确认" value={clearanceText(item.latest_clearance_status)} tone={item.latest_clearance_status === 'confirmed' ? 'ready' : item.latest_clearance_status === 'returned' ? 'failed' : 'pending'} /> : null}<DataLimitations quality={item.quality} /></td>
...
function notificationText(status: string | null, error: string | null) { if (status === 'sent') return '钉钉已受理'; if (status === 'pending') return '待发送'; if (status === 'failed') return error === 'oa_not_configured' ? '未配置 · 发送失败' : '发送失败'; return '尚未发送' }
