...
### 对架构师评审中“AC-F09-09/10 稳定语义冲突”的处理

- 架构师建议：PM 保留 AC-F09-09 为成功授权、AC-F09-10 为拒绝授权，工程稿随 PM 语义对齐。
- 我的处理：✅ 接受；§10 主流程与异常分支保留该唯一分配，并收紧各自触发和结果。
- 修订位置：AC-F09-09、AC-F09-10。

### 对架构师评审中“恢复与外部通知混为原子结果”的处理

- 架构师建议：拆成“恢复已安全完成”和“安全通知处置已闭环”，投递失败不回滚凭据。
- 我的处理：✅ 接受；明确内部事务失败才保持旧凭据，异步投递失败形成可见待处理与告警。
- 修订位置：AC-F09-06。

### 对架构师评审中“日常配置时间不可取数”的处理

- 架构师建议：不得用 Admin 提交时间冒充材料可用时间。
- 我的处理：✅ 接受；采用 Admin 明确确认材料可用于配置时的服务端时间，并记录来源与可信边界，不新增审批状态或 OA 集成。
- 修订位置：§8.2、§8.3。

### 对架构师评审中“错误授权为 0 无法全由系统证明”的处理

- 架构师建议：区分系统阻止的非法授权与经人工确认的错授事件。
- 我的处理：✅ 接受；保留总护栏为 0，同时明确人工事件台账和“观察期内无已确认事件”的含义。
- 修订位置：§0.1、§8.2、§10.13。

### 对架构师评审中“审计覆盖率缺少独立分母”的处理

- 架构师建议：以领域事实为分母，登录拒绝等无状态变更事件另行核对。
- 我的处理：✅ 接受；不可计算时显示证据不完整，不允许天然得出 100%。
- 修订位置：§8.2。

### 对架构师评审中“闭环完成率缺少 cohort 与截止日”的处理

- 架构师建议：按授权周期统计，并明确观察窗与未完成原因。
- 我的处理：✅ 接受；固定上线日起 30 个自然日，待处理、冲突、停用均保留在分母并分原因展示。
- 修订位置：§8.2。

### 对架构师评审中“价值归因缺少数据来源边界”的处理

- 架构师建议：由产品外岗位人工勾稽，不暗增价值分析后台。
- 我的处理：✅ 接受；F09 只提供稳定授权周期及关键事件引用。
- 修订位置：§1.4、§8.2。

### 对工程师评审中“保存型恢复码过早消费”的处理

- 工程师建议：新 TOTP 实码通过后，旧恢复码消费、凭据换新、会话撤销和审计才同时成立。
- 我的处理：✅ 接受；失败不产生半套凭据或提前消费恢复码。
- 修订位置：AC-F09-05。

### 对工程师评审中“AC-F09-06 外部通知伪原子”的处理

- 工程师建议：区分内部安全恢复提交与外部通知处置。
- 我的处理：✅ 接受；与架构师同类意见合并落入同一 AC，但保留两种连续可观察状态。
- 修订位置：AC-F09-06。

### 对工程师评审中“AC-F09-09 同时包含授权和后续登录”的处理

- 工程师建议：AC-F09-09 只保留 Admin 授权提交结果，后续登录由既有 AC-F07-05 与 AC-F09-15 承载。
- 我的处理：✅ 接受；删除第二主体的后续登录结果，并显式保持旧 AC 兼容。
- 修订位置：AC-F09-07、AC-F09-09、AC-F09-13、AC-F09-15。

### 对工程师评审中“AC-F09-10 审批人与目标身份唯一对应语义错误”的处理

- 工程师建议：唯一对应对象应是审批证据中的目标人员与当前目标身份；审批人另验责任范围。
- 我的处理：✅ 接受。
- 修订位置：AC-F09-10。

### 对工程师评审中“AC-F09-12 并发结果不唯一”的处理

- 工程师建议：分别定义角色写并发、登录与角色操作并发、网络重放的唯一结果。
- 我的处理：✅ 接受；一个稳定 AC 下列出三种互斥触发分支，不新增 ID。
- 修订位置：AC-F09-12。

### 对工程师评审中“§0 与 AC-F09-15 上线门禁不一致”的处理

- 工程师建议：§0 保留唯一完整清单，AC-F09-15 仅在全部门禁满足时计时有效。
- 我的处理：✅ 接受；§0.2 统一为十项门禁，AC 不再用部分前置清单替代完成门禁。
- 修订位置：§0.2、§0.3、AC-F09-15。

### 对工程师评审中“AC-F09-15 关键时间点未固定”的处理

- 工程师建议：固定初始化开始、Admin 创建、首次正式登录、待处理身份首次产生、有效审批材料可用、角色配置成功、首次正确工作台访问七个里程碑。
- 我的处理：✅ 接受；明确服务端口径、首次/最近时间边界及缺失节点结果。
- 修订位置：§8.3、AC-F09-15。

### 对工程师评审中“高权限审计覆盖率分母不明确”的处理

- 工程师建议：覆盖率按应审计实例计算，而非事件类别是否至少出现一次。
- 我的处理：✅ 接受并进一步采用领域事实作独立分母；无领域状态变更的拒绝事件单列核对。
- 修订位置：§8.2。

### R3 不变边界确认

- 未新增 `OPEN_QUESTION`；两份 R2 评审均确认可依据既有输入收敛。
- 未新增或重排稳定 ID，仍只使用 F09、US-F09-01—03、REQ-F09-01—07、AC-F09-01—15、NFR-006—010。
- 单 Admin、独立本地认证、TOTP、Admin 与业务角色正交、事业部负责人审批而 Admin 只核验执行的边界不变。
- 未新增多 Admin、OA/HR/IDaaS、批量授权、可配置角色、统计大屏、价值分析后台或未确认 SLA。
- F01—F08 的已有合法用户入口、经营数据、任务、结果、业务审计和稳定 ID 均保持不变。
