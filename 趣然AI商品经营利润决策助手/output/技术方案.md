...
> 技术方案版本：v1.8（Issue #5 内部状态流转）

## 1. 产品技术画像

本产品是玩具事业部内部使用的单端 PC Web 工作台。它把每周 XLSX 固化为不可变 SPU 快照，用固定规则生成经营/库存双动作；只有运营和运营主管登录产品，责任运营在产品内直接确认两轨执行，清仓完成时间经主管确认后闭环。钉钉只承担 OAuth 统一身份认证，不参与业务通知或状态推进。技术方案优先保证确定性、双角色权限、状态幂等和历史可重放。
...
| 双角色与内部状态最小可见 | REQ-F07-01～03、NFR-002 | 业务接口仅接受运营/主管；外部相关人员无产品访问权，库存动作不依赖外部账号或消息回执 |
...
| 清仓完成内部确认 | REQ-F06-05 | 运营提交实际完成时间、主管确认后闭环；待提交、待确认和待修正状态持续在站内可见 |
...
| `DATA-F06-01` | `action_item`、`action_result`、`effective_action_revision`、`clearance_completion` | 一个稳定任务最多有经营/库存两轨；责任运营统一执行；库存处理说明必填；清仓完成时间保留提交、确认/退回版本；review/execution version 单调递增。既有 `oa_notification` 只读保留历史，不再创建。 |
...
- Issue #5 后不再创建 OA 通知或每日催办；待处理库存动作与清仓确认只通过站内状态投影进入运营/主管工作台和行动清单。历史 `oa_notification` 与 `oa_delivery` 事件不删除、不重写，只在历史时间线中保留。
