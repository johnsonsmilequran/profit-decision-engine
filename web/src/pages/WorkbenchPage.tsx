...
export function WorkbenchPage({ expectedRole }: { expectedRole: 'operations' | 'supervisor' }) {
  ...
  }) ?? [], [action, businessState, query.data, search])
  ...
  return <AppShell active="workbench"><section data-page-id={supervisor ? 'PAGE-F06-01' : 'PAGE-F05-01'}>
    ...
    {query.isPending ? <State title="正在加载工作台" body="正在按当前身份读取真实任务。" /> : query.isError ? <State title="工作台加载失败" body="未显示任何可能过期的经营数据。" action={<button className="button" onClick={() => query.refetch()}>重新加载</button>} /> : query.data ? <>
      ...
      <div className="panel section-panel"><div className="section-title"><div><h2>{supervisor ? '待审核建议' : '优先待办'}</h2><p>{supervisor ? '按清仓、止损、观察、加投固定风险顺序展示；必须进入单条详情审核。' : `当前显示 ${filteredItems.length} 条本人任务；无前序时不生成虚构折叠入口。`}</p></div>{supervisor ? <a className="button" href={`/actions?batch_id=${query.data.latest_batch_id}&review_status=pending`}>查看全部待审核</a> : null}</div><ActionTable items={supervisor ? query.data.items : filteredItems} /></div>
      <div className="workbench-lower"><div className="panel section-panel"><h2>{supervisor ? '钉钉异常与任务去向' : '数据限制'}</h2>{supervisor ? <><div className={query.data.notification_failure_count ? 'notice warning' : 'success-note'}>{query.data.notification_failure_count ? `当前批次有 ${query.data.notification_failure_count} 条钉钉通知发送失败，业务状态未被自动推进。` : '当前批次没有钉钉发送异常。'}</div><p className="muted">动作变化 {query.data.exception_count} 条；并发冲突只追加审计，不覆盖已成功事实。</p><div className="heading-actions"><a className="table-link" href="/actions">进入行动中心</a><a className="table-link" href="/history">查看历史追溯</a></div></> : query.data.data_limitations.length===0 ? <p className="muted">本批次未发现数据限制。</p> : <><ul className="limitation-list">{query.data.data_limitations.map(item=><li key={`${item.field}-${item.status}`}><strong>{limitationField[item.field]??item.field}</strong><span>{limitationStatus[item.status]??item.status} · 影响 {item.count} 个 SPU</span></li>)}</ul><a className="table-link" href={`/batches/${query.data.latest_batch_id}`}>进入批次详情核对来源与影响</a></>}</div>{supervisor ? <RecentReviews items={allItems.data?.items ?? []} /> : <RecentOperations items={query.data.items} />}</div>
    </> : null}
  </section></AppShell>
}
