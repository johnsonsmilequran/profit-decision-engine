...
- status: active
- derived_decision: 新品年龄使用批次业务截止日以保证历史重放确定性；跨周动作变化时继承现有主管审核边界，相同动作免重复审核，变化动作待主管确认后生效；稳定经营待办以 SPU/商品链接锚定，每周关联指向最近前序待办记录并保留任务产生、状态与执行时间；这些是阶段 5 派生产品决策，不冒充用户原始事实
...
| 钉钉统一身份认证与三角色映射 | committed | 事业部负责人审批；运维/系统管理员配置 | 用户确认以钉钉为统一身份入口；三类角色变更由事业部负责人审批，运维/系统管理员按审批结果配置并维护；接入连通性待实施验证 | 不可用或无有效角色映射时拒绝进入业务页，不得降级为共享账号、账号密码或仅前端隐藏 | MVP | REQ-F07-01、REQ-F07-02、REQ-F07-03 |
...
- Revision: 6
- Status: active
- Behavior: 系统以 SPU/商品链接为经营待办锚点；当前批次存在历史经营待办时显示与最近一条前序待办记录的关联，并展示稳定任务首次产生时间、当前经营状态、经营动作执行时间以及本周关联时间；行动清单和运营待办视图支持按经营状态筛选。
...
THE SYSTEM SHALL 在当前经营待办中显示稳定任务 ID、首次产生时间、本周关联时间、当前经营状态和经营动作执行时间，并关联显示按业务时间倒序得到的最近一条前序待办记录，包括其来源批次、动作、状态、产生时间和执行时间；尚未执行时执行时间明确显示“—”，不得用更新时间替代。
```
...
THE SYSTEM SHALL 只显示当前经营动作状态匹配的 SPU 待办，并在刷新或从建议详情返回后恢复该筛选；经营状态筛选不得被建议审核状态、采购动作状态或建议总览状态替代。
```
...
- Role: 运营
- Goal: 与采购计划分别更新各自负责动作的状态和结果
- Value: 形成建议、审核、执行、结果的轻闭环
- Stage: MVP
- Status: active
...
### REQ-F06-02 · 按责任跟踪经营与采购动作

- Story: US-F06-02
- Stage: MVP
- Revision: 1
- Status: active
- Behavior: 运营更新经营动作，采购计划更新补货或禁补动作；两个动作状态独立但保持同一建议关联。
...
```text
WHEN 运营主管通过一条待审核建议
THE SYSTEM SHALL 保存一次建议级“已通过”结果，并同时激活该建议内已原子生成的经营动作与补货/禁补动作项，分别置为运营与采购计划待执行；不得在审核时二次创建动作项，主动作和补货/禁补不得产生相互矛盾的审核结果。
```
...
THE SYSTEM SHALL 只保存该经营动作的执行状态、时间、备注和动作结果；证据不足时，观察动作不得记录超出证据的具体优化项。
```
...
```text
WHEN 采购计划更新其权限范围内的补货或禁止补货动作
THE SYSTEM SHALL 只保存该采购动作的执行状态、时间、备注和结果；禁止补货可记录为“已确认禁补”或等价结果，不表示已收到外部系统回执。
```
...
THE SYSTEM SHALL 把结果周期、记录时间、记录人、可用结果值和备注关联到原建议；采购计划只能记录补货/禁补动作结果，不获得完整经营结果字段。
```
...
- Role: 采购计划
- Goal: 只查看并处理补货或禁补所需信息
- Value: 在不暴露完整利润、推广和售后数据的前提下完成采购动作
- Stage: MVP
- Status: active
...
### REQ-F07-02 · 限制采购数据与操作

- Story: US-F07-01
- Stage: MVP
- Revision: 1
- Status: active
- Behavior: 采购计划仅可见补货或禁补结论及必要库存、销量依据，并只处理补货或禁补动作；越权读取与操作必须被拒绝。
...
- Revision: 3
- Status: active
- Behavior: 系统使用钉钉作为统一身份认证入口，并读取经事业部负责人审批、由运维/系统管理员按审批结果配置的运营、运营主管、采购计划角色映射；无有效认证或无有效角色映射时拒绝进入业务页，产品不提供账号密码、注册、找回密码或业务侧角色管理入口。
...
```text
WHEN 采购计划直接访问任一建议或历史记录
THE SYSTEM SHALL 先校验该建议是否包含分配给当前采购角色且其有权查看的补货/禁止补货任务：有任务关联时只显示 SPU 身份、补货/禁止补货结论、库存与销量必要依据及其动作状态；无任务关联或无访问权时显示权限不足，且不返回建议身份或状态字段。
```
...
```text
WHEN 采购计划通过页面、筛选、接口、错误信息或审计详情请求完整利润、推广、品退或售后信息
THE SYSTEM SHALL 不得向其展示或返回这些内容。
```
...
```text
WHEN 钉钉认证失败、登录态失效，或运维/系统管理员未按审批结果配置运营、运营主管、采购计划中的有效角色
...
- Revision: 2
- Status: active
- Measure: 采购计划的页面、接口响应和错误路径中不得出现完整利润、推广和售后字段，仅出现补货或禁补所需的商品、库存和销量依据。
...
- Applies-to: REQ-F01-01、REQ-F05-01、REQ-F05-04、REQ-F05-05、REQ-F06-01、REQ-F06-02、REQ-F06-03、REQ-F06-04、REQ-F08-01、REQ-F08-02
- Revision: 1
- Status: active
