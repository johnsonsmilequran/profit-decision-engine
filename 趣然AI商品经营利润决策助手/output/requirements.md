...
### REQ-F07-03 · 使用钉钉认证并按主管授权映射业务角色

- Story: US-F07-01
- Stage: MVP
- Revision: 8
- Status: active
- Behavior: 系统使用钉钉作为统一身份认证入口，MVP 只由本机普通浏览器的点击操作发起 OAuth，不承诺扫码登录或钉钉客户端/工作台入口；运营主管可在产品内以钉钉 `unionId` 为唯一身份新增或维护运营、运营主管角色映射并启用或停用账号；无有效认证或无有效角色映射时拒绝进入业务页。产品不提供采购、仓库等外部协同角色、账号密码、注册或找回密码。

### REQ-F07-04 · 主管管理用户角色且避免权限锁死

- Story: US-F07-01
- Stage: MVP
- Revision: 1
- Status: active
- Behavior: 只有运营主管可查看和修改角色映射；变更写入真实 PostgreSQL 并记录批准人、配置人和时间。主管不得停用自己或把自己降为运营，系统不得停用或降级最后一个有效主管；运营和未认证用户访问管理页面或接口均被拒绝。
...
```text
WHEN 运营主管提交合法的钉钉 unionId、显示名、运营或主管角色及启用状态
THE SYSTEM SHALL 在单一数据库事务中新增或更新唯一角色映射，记录当前主管为批准人和配置人，并使后续认证按最新角色生效。
```
...
```text
WHEN 运营、未认证用户或未知角色访问角色管理页面或接口
THE SYSTEM SHALL 拒绝访问且不返回角色映射列表，不执行任何写入。
```
...
```text
WHEN 主管尝试停用自己、把自己降为运营，或停用/降级最后一个有效主管
