...
export function ActionListPage() {
  ...
  return <AppShell active="actions"><section data-page-id="PAGE-F05-02">
    ...
    <div className="filter-panel action-filter-strip">
      ...
      <label>库存状态<select value={filters.inventoryState} onChange={event => update({ inventoryState:event.target.value })}><option value="">全部</option><option value="pending_review">待审核</option><option value="pending_execution">待处理</option><option value="processed">已处理</option><option value="not_generated">未生成库存动作</option><option value="closed">已关闭</option><option value="terminated">已终止</option></select></label>
