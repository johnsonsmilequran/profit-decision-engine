...
运营与运营主管统一通过钉钉完成身份认证，每人使用自己的钉钉身份登录；运营主管通过产品内“用户角色”页维护钉钉身份与业务角色的映射，系统禁止主管停用/降级自己或移除最后一个有效主管。采购、仓库等相关人员只接收 OA 协同消息，不建立产品角色。产品不提供账号密码、注册或找回密码；无有效认证或无有效映射时默认拒绝进入业务页。登录后全局账号菜单显示本人姓名、角色及完整 unionId，并提供复制与退出登录；退出须撤销服务端会话，允许用户重新发起钉钉登录。
...
**位置**：钉钉认证入口与登录回跳、全局账号菜单、主管用户角色管理页、批次、行动清单、建议详情、审核、分动作执行和追溯入口；不提供账号密码或普通运营权限配置页。
...
| 当前账号 | 全局账号菜单 | 显示姓名、角色和完整 unionId | 可复制 unionId 或退出登录 | 未登录时不显示 |
| 用户角色管理 | 主管专属页面 | 显示完整 unionId、显示名、角色和状态 | 可复制 unionId、新增或修改映射 | 普通运营不可访问 |
...
6. 运营主管在产品“用户角色”页新增、修改或停用映射；服务端禁止主管停用/降级自己或移除最后一个有效主管，并使角色变化后的旧会话失效。
7. 登录用户可在全局账号菜单查看、复制本人完整 unionId；主管在用户角色页可查看、复制被管理账号的完整 unionId。unionId 不进入 URL、日志或未认证页面。
8. 用户退出登录时，系统撤销服务端会话、清除 HttpOnly Cookie 并返回登录页；旧 Cookie 不得继续访问业务 API。
9. deployment secret 只由运维在部署环境维护；页面、响应、错误和审计均不返回密钥值。
...
### AC-F07-10（关联 REQ-F07-03）

当已登录用户打开全局账号菜单时，系统应显示其姓名、业务角色及完整 unionId，并允许复制；当用户退出时，系统应撤销服务端会话、清除 HttpOnly 会话 Cookie 并返回登录页，原 Cookie 后续访问业务 API 必须被拒绝。unionId 不得写入 URL、日志或未认证页面。

### AC-F07-11（关联 REQ-F07-04）

当运营主管打开用户角色管理页时，系统应显示每个映射账号的完整 unionId 并提供复制操作，以支持将真实钉钉账号与业务角色准确关联；普通运营不得访问该页或接口。
