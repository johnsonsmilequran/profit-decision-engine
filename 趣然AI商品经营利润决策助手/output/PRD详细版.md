...
本产品不是单角色分析工具。运营与运营主管需要完整经营信息完成执行和审核；采购计划只接收补货相关最小必要信息。建议由运营主管一次审核；若主管不采纳固定规则动作，可在任一动作开始执行前建立人工改判版本。改判只改变“生效动作”，原规则结论与冻结证据永久保留；清仓改为加投时必须同步重新确认库存动作，不得沿用禁补。建议通过后，经营与库存生效动作分别由运营和采购计划执行并记录结果。
