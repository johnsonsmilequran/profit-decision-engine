...
> 实现同步（2026-08-04）：`API-F06-03/04` 已落地为 Go 事务命令、PostgreSQL 版本/幂等保护与 React 建议详情表单。人工改判直接新增 `supervisor_override` 生效版本，固定规则版本只转为历史状态而不删除；任一轨已执行时必须先调用 `terminate` 追加终止事件。清仓完成由责任运营提交后保持原执行态，主管通过独立 `confirm` 或 `return` 命令确认/退回，只有确认才将经营轨关闭；退回原因和再次提交均保留版本。
