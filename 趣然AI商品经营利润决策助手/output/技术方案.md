...
> 环境分层实现同步（2026-08-07）：开发基础设施、开发应用与线上部署使用物理独立入口。`infra/compose.yaml` 只运行 PostgreSQL，使用 `profit-decision-dev-infra` 项目名、独立开发卷和 `infra/.env`；React、Go API 与 Go Worker 通过 `.env.development` 在本机运行。根 `compose.yaml` 与根 `.env` 只用于线上形态的 PostgreSQL、迁移、API、Worker、Web 全栈部署。禁止从线上 Compose 选择性启动 `db` 来冒充开发 infra，也禁止开发进程读取线上环境文件；Makefile 以 `infra-*`、`dev-*`、`prod-*` 明确环境，含糊的 `compose-up` 入口必须失败并提示改用明确命令。
...
> 实现同步（2026-08-07）：仓库已建立 `web/`、`backend/`、`deploy/`、独立 `infra/compose.yaml`、根线上 `compose.yaml` 与 `Makefile`。首个纵向切片使用 PostgreSQL `role_mapping/user_session`、真实钉钉 OAuth 服务端适配器、HttpOnly 会话及 React `/login`、`/auth/recovery`；缺少部署密钥时保持默认拒绝，不提供本地账号或认证绕过。钉钉适配器使用生产官方端点，HTTP 契约测试只注入隔离测试端点，验证授权参数、授权码换 token、token 换稳定 unionId 及各类非 2xx/缺字段/非法 JSON 默认拒绝。MVP 普通浏览器登录页只保留一个指向 `/auth/dingtalk/start` 的点击按钮；不生成扫码，不实现或验收钉钉客户端/工作台入口。会话生命周期由真实 PostgreSQL 集成测试覆盖运营/主管唯一角色、无角色、角色变化、角色停用、闲置 2h、绝对 12h、撤销；每次认证重新对照当前角色映射。开发 infra 的数据库使用独立 Compose 项目和卷，并默认映射本机 `55432` 供本机 API/Worker 使用；线上 Compose 内数据库只使用容器网络 `5432`。PostgreSQL 18 持久卷按官方镜像的主版本目录结构挂载到 `/var/lib/postgresql`。工程入口以根 README 与 Makefile 为准，后续闭环继续沿用同一目录和事务边界。
