...
- latest_development_check: `2026-08-04 18:48`，F06 四个 React 表单路由已接入真实详情/写 API，角色与合法状态控制详情入口；Web 8/8、类型检查和生产构建通过，生产包机械包含 PAGE-F06-01～04。真实数据库定向集成覆盖整体通过/驳回、幂等、旧 version、双轨独立执行、部分结果及采购越权。四页浏览器默认态与布局尚待开发期检查；该证据未进入发布候选，以下状态保持 pending。
