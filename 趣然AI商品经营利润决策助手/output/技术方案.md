...
> 实现同步（2026-08-04）：Go Worker 已通过 `LITELLM_BASE_URL`、`LITELLM_API_KEY`、`LITELLM_MODEL` 调用兼容 OpenAI `POST /chat/completions` 的现有 LiteLLM 网关，API 进程不接收模型密钥。每条新决策先持久化 `generating` 版本，再由 Worker 读取原冻结快照生成；重试只新增版本。响应严格限制为 `problem/evidence/action/summary` 四个非空字符串，未知字段、动作不等于固定规则动作组合、出现输入白名单之外数字，或包含 SKU/广告明细/评价与退款归因时整段记为 `not_adopted` 且不保存原文。网络/网关失败记受控失败码，最新失败或生成中状态仍读取上一版合规内容，不改变审核、经营或库存状态。
