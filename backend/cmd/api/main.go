...
func main() {
	...
	server := &http.Server{Addr: cfg.Address, Handler: httpapi.New(db, identities, dingTalk, cfg.PublicBaseURL, cfg.CookieSecure, logger), ReadHeaderTimeout: 5 * time.Second}
	go func() {
		<-ctx.Done()
