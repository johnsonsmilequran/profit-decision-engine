...
> 实现同步（2026-08-04）：已认证用户对真实任务发起无权命令或旧版本命令时，HTTP 仍分别返回 403/409，同时在独立事务追加 `authorization_denied` / `version_conflict` 事件；审计写入不改变任务状态，且审计失败不得遮蔽原业务响应。历史投影只计数并按权限展示这些异常事件。
