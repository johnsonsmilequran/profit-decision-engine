...
> 验收实现同步（2026-08-04）：XLSX 解析与批次集成测试在提供 `TEST_XLSX_PATH` 时读取该真实文件；本轮正式候选使用仓库根目录用户提供的 `商品链接.xlsx`。测试未配置外部文件时由 Excelize 生成包含 10 条中文经营数据的真实 `.xlsx` 以避免跳过解析器，但该夹具不替代正式验收输入。各 PostgreSQL 集成测试自行建立所需角色与领域记录，不依赖其他测试包的执行顺序。
