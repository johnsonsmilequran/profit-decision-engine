...
- 契约：revision 10 的 26 REQ、52 AC、5 NFR（共 83 个 source）全部必须出现于追溯矩阵且不得 stale/TBD；`REQ-F05-05` 与 `AC-F05-09` 的 revision 8 行为继续完整验证。
...
正式进入 TDD Master 时必须把以下 revision 10 验收面设为 required；revision 7 的双角色/OA/清仓确认和 revision 8 的前序同构行用例继续保留，不得仅复用更早的双轨用例：
...
# Issue #5 增量影响（2026-08-05）

## 变更结论

- 删除钉钉机器人业务通知、公司 User ID 输入、每日催办及人工补发流程。
- 保留钉钉 OAuth 统一身份认证；本 Issue 不改变运营/运营主管角色映射和服务端会话。
- 库存动作复用现有带 version、幂等键和审计事件的内部执行命令，由责任运营填写非空处理说明后确认“已处理”。
- 清仓完成继续使用运营提交、主管确认/退回的内部状态链；待办通过产品工作台和行动清单持续可见。
- 既有 `oa_notification` 历史表和旧记录只读保留，不再创建新记录，避免破坏既有审计证据。

## 影响链

`REQ-F06-02/F06-05 → PAGE-F05-01/PAGE-F05-02/PAGE-F06-01 → API-F06-02/SEQ-F06-01/02 → FLOW-09/FLOW-11/DESIGN-02/04/07`

## 不变行为

- 固定规则、整体审核、双轨独立 version、重复提交幂等、旧 version 冲突、历史快照和事件追加不变。
- 钉钉 OAuth 登录、无角色默认拒绝、LiteLLM 独立降级不变。
