...
- status: active
- derived_decision: 新品年龄使用批次业务截止日以保证历史重放确定性；跨周动作变化时继承现有主管审核边界，相同动作免重复审核，变化动作待主管确认后生效；稳定经营待办以 SPU/商品链接锚定，每周关联指向最近前序待办记录并保留任务产生、状态与执行时间；清仓完成时间采用“运营提交、运营主管确认”双时间并以主管确认作为闭环条件；这些是阶段 5 与设计增量派生产品决策，不冒充用户原始事实
