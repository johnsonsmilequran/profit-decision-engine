...
> 变更日期：2026-08-04  
> 上游需求契约：`requirements.md` revision 6 → 7
> 变更性质：业务角色收敛为运营与运营主管；外部相关人员改由责任运营通过 OA 协同；新增清仓完成时间双人确认与每日幂等催办。
...
| design-derived：最近前序待办折叠展示 | N/A（revision 7 设计增量） | `PAGE-F05-01`、`PAGE-F05-02`、`PAGE-F06-03`、`CMP-PREVIOUS-TODO-LINK` | 设计系统、蓝图、页面清单；运营工作台/行动清单/建议详情 MD+HTML；追溯矩阵；TDD 下游验证面 | 当前任务身份、动作、状态和生命周期常驻；最近前序来源批次、动作、状态、产生/执行时间按 SPU 默认折叠，点击或键盘展开；首次无前序不渲染空折叠 | implemented |
...
- 本次仅调整已有前序信息的披露层级，`PRD详细版.md` 与 `requirements.md` revision 7 的数据关联、权限、状态机和验收语义不变。

## 验证面

- 契约：revision 7 的 26 REQ、52 AC、5 NFR（共 83 个 source）全部必须出现于追溯矩阵且不得 stale/TBD。
- 页面：双角色工作台、行动清单与建议详情显示责任运营、OA 协同、清仓完成确认与最近催办；行动清单与运营工作台可按经营状态筛选。
- 回归：页面 ID、JavaScript 语法、既有链接和不受影响页面保持有效。

## TDD 下游同步

正式进入 TDD Master 时必须把以下 revision 7 验收面设为 required，不得仅复用 revision 6 的双轨用例：
...
| 最近前序折叠披露 | 当前任务信息始终可见；有前序时入口默认收起，鼠标点击或键盘 Enter/Space 只展开当前 SPU 的来源批次、动作、状态、产生/执行时间且不触发行跳转；首次无前序不出现空折叠；刷新、筛选、分页和详情返回后恢复折叠 |
