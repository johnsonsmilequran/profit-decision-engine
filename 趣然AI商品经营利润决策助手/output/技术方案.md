...
- 每日催办以 `action_task_id + local_date + recipient_actor_ref + template_code` 建唯一约束，其中现有数据库列 `recipient_actor_ref` 存储钉钉公司 User ID；钉钉接口受理只更新通知状态，不得写入送达/已读或业务确认时间。清仓完成已确认、动作改判或任务终止时停止新建提醒。
