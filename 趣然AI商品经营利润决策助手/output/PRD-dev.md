...
| FR-5 / F05 | 行动清单 | 每批一份、固定排序、筛选、历史保留、同 SPU 跨周任务续接 | 纯维持且无库存动作默认不进入清单；同动作不重建任务 |
...
| SpuActionTask | 事业部、SPU、稳定任务 ID、当前生效动作版本 | 跨批次续接；每周决策通过关联表指向当时版本 |
...
完整场景见 `output/requirements.md` 的全部 active AC，包括认证场景 AC-F07-05 至 AC-F07-06 与跨周任务场景 AC-F05-06 至 AC-F05-08。开发必须至少先走通：
