...
- Revision: 1
- Status: active
- Behavior: 系统支持仅替代一次 TOTP 的保存型恢复码和经独立审批配置的部署恢复口令；恢复成功后换新凭据代次、撤销旧管理会话与旧恢复材料、追加审计并写入可追踪通知 outbox。
...
- Revision: 1
- Status: active
- Behavior: Admin 仅在稳定作用域身份、至少两个可靠组织属性、事业部负责人审批依据、唯一目标角色和当前映射版本均有效时，新增、变更、停用或恢复运营/运营主管角色；角色、审批、业务会话撤销、五态投影与管理审计原子变更。
...
THE SYSTEM SHALL 只创建一名 Admin，原子消费 setup 材料、追加初始化审计、关闭 setup 入口并引导使用新凭据正式登录。
```
...
```text
WHEN setup 材料无效、过期、已消费、环境不匹配，或初始化发生重复、并发、刷新、网络中断或结果未知
THE SYSTEM SHALL 不泄露精确材料状态且不产生半成品或第二 Admin；已有成功结果时返回完成状态。
```
...
```text
WHEN Admin 用户名和密码正确并提交未消费的保存型恢复码，随后完成新 TOTP 实码验证
THE SYSTEM SHALL 在提交事务内消费旧恢复码、换新 TOTP 与恢复码、递增凭据代次、撤销旧 Admin 会话并追加审计；提交前失败时旧恢复码仍可用。
```
...
```text
WHEN 正常管理凭据全部不可用且恢复执行人提供经独立审批、当前环境有效的部署恢复材料并完成新密码和 TOTP 设置
THE SYSTEM SHALL 原子换新凭据代次、撤销旧会话、消费恢复材料、追加审计并持久化通知 outbox；外部投递或补偿结果独立追踪，失败不得伪装闭环或反向回滚安全恢复。
```
...
```text
WHEN 待处理身份的角色配置事务成功
THE SYSTEM SHALL 显示已配置待重新登录和唯一目标角色，撤销受影响旧业务会话，并在该身份重新通过钉钉认证后进入对应工作台。
```
...
THE SYSTEM SHALL 拒绝授权并保持原角色与原会话事实不变，显示缺失项和责任路径，且不得按姓名、备注、首位用户或 AI 猜测角色。
```
...
```text
WHEN 同一作用域身份发生并发角色操作或角色变更与钉钉回调并发
THE SYSTEM SHALL 依据映射版本只允许一个角色事务成功，失败请求不得覆盖成功结果，且业务会话只能绑定最终有效映射版本。
```
...
```text
WHEN Admin 会话访问业务接口、业务会话访问管理接口或任一方尝试身份转换
THE SYSTEM SHALL 拒绝请求且不返回对方域数据；同一自然人同时持有两类登录态时也必须分别鉴权。
```
...
THE SYSTEM SHALL 追加不含原始秘密的管理审计；Admin 只能查询且不能修改或删除，需审计的高权限事务在审计失败时不得成功。
```
...
THE SYSTEM SHALL 记录唯一 Admin 初始化开始、初始化提交、首次正式登录、待处理申请、配置开始、角色配置成功、正确工作台首次访问七个服务端里程碑；首尾不超过 10 分钟时才显示达成，任一节点缺失时不得填 0 或宣称通过。
```
...
- Revision: 1
- Status: active
- Measure: Admin 与业务身份使用独立入口、凭据、Cookie、会话、秘密和中间件；两类会话互访均被拒绝且不返回对方域数据。
...
- Revision: 1
- Status: active
- Measure: 密码、TOTP、setup/recovery 原始材料、恢复码和令牌不进入前端包、响应、日志、镜像、仓库或通用默认值；一次性材料按环境和凭据代次失效。
...
- Revision: 1
- Status: active
- Measure: 所有需审计的高权限领域变化均能与唯一追加审计事件勾稽；Admin 无更新或删除能力，审计失败时变化不成功。
