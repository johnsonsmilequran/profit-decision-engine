...
- latest_development_check: `2026-08-04 18:34`，API 30/30、Web 8/8、双端类型检查和生产构建通过；真实数据库 F05 集成覆盖固定排序、-18.6% 冻结快照、清仓+禁补双轨、四要素、采购列表/详情字段负向检查及自然月日期不偏移。生产包机械确认包含 PAGE-F05-01/02 且不含旧详情路由。1024px 浏览器检查主管/运营完整视图与采购最小视图均无整页横向溢出，采购页面无利润、经营动作、审核筛选和 AI 区。该证据未进入发布候选，以下状态保持 pending。
