...
function OperationsOverview({ items, data }: { items: ActionItem[]; data: Awaited<ReturnType<typeof getWorkbench>> }) { return <div className="operations-overview"><div className="panel section-panel"><div className="section-title"><div><h2>优先经营待办</h2><p>可行动任务优先；当前显示 {items.length} 条本人任务。</p></div><a className="table-link" href="/actions">查看全部行动</a></div><ActionTable items={items} /></div><aside className="operations-side"><section className="panel section-panel"><h2>待补录经营结果</h2><strong className="side-metric">{data.pending_result_count}</strong><p className="muted">已执行但尚未记录结果的经营动作；当前无待补录时保持可信空态。</p></section><section className="panel section-panel"><h2>数据限制</h2>{data.data_limitations.length===0 ? <p className="muted">本批次未发现数据限制。</p> : <><ul className="limitation-list">{data.data_limitations.slice(0,3).map(item=><li key={`${item.field}-${item.status}`}><strong>{limitationField[item.field]??item.field}</strong><span>{limitationStatus[item.status]??item.status} · {item.count}</span></li>)}</ul><a className="table-link" href={`/batches/${data.latest_batch_id}`}>核对全部来源与影响</a></>}</section></aside></div> }
