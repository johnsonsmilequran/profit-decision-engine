...
- latest_development_check: `2026-08-04 18:41`，新增真实数据库 F06 定向集成：主管整体通过同时激活既有双轨 action_id，同幂等键返回首次响应，旧 review_version 冲突；运营与采购分别执行后另一轨状态/version 不变；运营记录含周期的部分经营结果，采购越权被拒；空白驳回原因失败、合法驳回关闭既有动作。定向 API 3/3 与类型检查通过。F05 的 API 30/30、Web 8/8、构建与 1024px 证据仍为开发期参考。该证据未进入发布候选，以下状态保持 pending。
