...
> 实现同步（2026-08-04）：仓库已建立 `web/`、`backend/`、`deploy/`、根 `compose.yaml` 与 `Makefile`。首个纵向切片使用 PostgreSQL `role_mapping/user_session`、真实钉钉 OAuth 服务端适配器、HttpOnly 会话及 React `/login`、`/auth/recovery`；缺少部署密钥时保持默认拒绝，不提供本地账号或认证绕过。钉钉适配器使用生产官方端点，HTTP 契约测试只注入隔离测试端点，验证授权参数、授权码换 token、token 换稳定 unionId 及各类非 2xx/缺字段/非法 JSON 默认拒绝。普通浏览器登录页的按钮与二维码共用同一个 `/auth/dingtalk/start` 入口；二维码由 React 依据当前部署域名和返回地址实时生成，不保存演示二维码、不引入第二认证路径。会话生命周期由真实 PostgreSQL 集成测试覆盖运营/主管唯一角色、无角色、角色变化、角色停用、闲置 2h、绝对 12h、撤销；每次认证重新对照当前角色映射。Compose 内数据库使用 `5432`，默认映射本机 `55432`以支持 API/Worker 独立启动与真实持久化冒烟；PostgreSQL 18 持久卷按官方镜像的主版本目录结构挂载到 `/var/lib/postgresql`。工程入口以根 README 与 Makefile 为准，后续闭环继续沿用同一目录和事务边界。
