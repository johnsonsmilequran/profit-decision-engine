...
| 钉钉统一身份认证与角色映射 | 已选定，接入可用性待实施验证 | IT | 用户确认以钉钉为统一身份入口，并由 IT 维护运营、运营主管、采购计划角色映射 | 认证失败或无有效映射时拒绝进入业务页；不得降级为共享账号、账号密码或仅前端隐藏 |
...
三类直接用户统一通过钉钉完成身份认证，每人使用自己的钉钉身份登录；运营、运营主管、采购计划角色映射由 IT 维护。产品只消费认证结果与角色映射，不提供账号密码、注册、找回密码或业务侧角色管理后台；无有效认证或无有效映射时默认拒绝进入业务页。
...
| 元素 | 类型 | 默认态 | 操作后 | 禁用条件 |
|---|---|---|---|---|
| 钉钉认证入口 | 外部统一身份入口 | 未登录时显示 | 成功后返回产品并建立登录态 | 钉钉认证不可用时显示失败与联系 IT 指引 |
...
1. 未登录用户只可发起钉钉统一身份认证；认证成功后系统读取 IT 维护的唯一有效业务角色并建立登录态。
...
6. 角色映射由 IT 在钉钉身份体系中维护；产品不提供业务侧角色编辑入口。
...
- 钉钉身份没有 IT 配置的有效角色映射：显示“未配置业务角色，请联系 IT”，默认拒绝且不自动赋予运营权限。
...
- 首版不提供账号密码、注册或找回密码；钉钉是唯一统一身份入口，角色映射由 IT 维护。
...
| 钉钉统一身份认证 | 为每位直接用户提供独立身份，并承载三类业务角色映射来源 | 公司钉钉身份由 IT 管理 | IT 维护运营、运营主管、采购计划角色映射及接入配置 | 钉钉身份体系与本产品服务端认证配置；产品无业务侧角色后台 | 玩具事业部直接用户 | IT 维护；实施阶段验证连通性 | 认证失败、登录态失效或无映射时默认拒绝并引导联系 IT，不降级为共享账号或账号密码 | REQ-F07-01～REQ-F07-03；AC-F07-01～AC-F07-06；NFR-002、NFR-005 |
...
| 钉钉认证与三类操作角色映射能力 | 已选定，接入可用性待实施验证 | IT | 用户确认钉钉统一身份入口及 IT 映射维护责任；需验证能记录稳定 actor_ref | 不可用或无映射时拒绝进入业务页，不得以共享账号、账号密码或前端隐藏开放真实数据 | REQ-F06-01～REQ-F07-03；AC-F06-01～AC-F07-06；NFR-002、NFR-005 |
...
当用户通过钉钉完成统一身份认证且 IT 已为其配置唯一有效业务角色时，系统应建立与该钉钉身份关联的登录态，显示当前业务角色，并在页面、接口、错误和历史路径统一应用该角色的读取与操作权限。

### AC-41（AC-F07-06，关联 REQ-F07-03）

当钉钉认证失败、登录态失效，或 IT 未配置运营、运营主管、采购计划中的有效角色时，系统应拒绝进入业务页且不返回任何经营数据，并提供重新发起钉钉认证或联系 IT 的恢复指引；不得自动赋予默认角色或降级为共享账号、账号密码登录。
...
| 用户关于身份认证的确认 | 钉钉是统一身份入口；运营、运营主管、采购计划角色映射由 IT 维护 | B |
...
| 身份认证与角色映射 | 已确认 | 使用钉钉统一身份认证，IT 维护三类角色映射；产品不增加账号密码或业务侧权限后台 |
...
| 身份与角色来源 | 钉钉统一认证、IT 维护三类角色映射、无映射默认拒绝 | PASS |
