...
| 钉钉统一身份认证与三角色映射 | committed | IT | 用户确认以钉钉为统一身份入口，并由 IT 维护运营、运营主管、采购计划角色映射；接入连通性待实施验证 | 不可用或无有效角色映射时拒绝进入业务页，不得降级为共享账号、账号密码或仅前端隐藏 | MVP | REQ-F07-01、REQ-F07-02、REQ-F07-03 |
...
### REQ-F07-03 · 使用钉钉认证并由 IT 映射业务角色

- Story: US-F07-01
- Stage: MVP
- Revision: 2
- Status: active
- Behavior: 系统使用钉钉作为统一身份认证入口，并读取由 IT 维护的运营、运营主管、采购计划角色映射；无有效认证或无有效角色映射时拒绝进入业务页，产品不提供账号密码、注册、找回密码或业务侧角色管理入口。
...
```text
WHEN 用户通过钉钉完成统一身份认证且 IT 已为其配置唯一有效业务角色
...
```text
WHEN 钉钉认证失败、登录态失效，或 IT 未配置运营、运营主管、采购计划中的有效角色
THE SYSTEM SHALL 拒绝进入业务页且不返回任何经营数据，并提供重新发起钉钉认证或联系 IT 的恢复指引；不得自动赋予默认角色或降级为共享账号、账号密码登录。
```
