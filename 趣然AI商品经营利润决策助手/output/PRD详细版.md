...
> **Issue #5 增量基线（2026-08-05，优先于本文旧版 OA 业务描述）**：补货、禁补等库存动作只在产品内部流转。责任运营必须在建议详情填写实际处理说明后确认，系统记录操作者、时间、版本与审计事件；不再要求钉钉 User ID，不发送机器人业务通知，不生成每日催办，也不提供人工补发。清仓完成仍由运营提交、主管确认或退回，并持续显示为站内待办。既有 OA 通知记录仅作历史只读证据；钉钉 OAuth 登录不受影响。
...
| 角色与执行责任 | 已确认并写入稳定需求契约 | 运营主管、运营 | 运营统一负责加投、观察、SPU 推广止损、清仓、补货/禁补库存动作及结果闭环；运营主管负责建议审核、改判和清仓完成时间确认 | 不自动改广告、采购或库存系统；库存动作只记录产品内部执行事实 |
...
| 钉钉企业内部机器人单聊 | Issue #5 起退出产品业务流程 | — | 历史投递结果仅保留为审计证据，不再创建新通知 | 不影响钉钉 OAuth 身份认证；业务执行全部改为站内状态流转 |
...
3. **动作级执行状态**：建议通过后，经营动作与补货/禁补库存动作均归该 SPU 责任运营；两类动作分别记录状态、必填说明、操作者、时间和版本，全部在产品内部闭环。
