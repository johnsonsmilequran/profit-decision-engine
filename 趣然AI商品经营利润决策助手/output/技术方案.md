...
> 行动清单实现同步（2026-08-04）：`GET /api/actions` 与 React `/actions` 已实现待我处理/全部/处理中/已完成四页签，支持批次、SPU、经营或库存动作、店铺、责任运营、审核、经营、库存、清仓确认与任务总进度组合筛选。默认页签按运营/主管责任分别投影，筛选与页签写入 URL；历史批次入口强制追加 `mode=history` 并隐藏写操作。非法枚举筛选回到安全默认，不改变业务数据。工作台先独立解析最新 ready 批次，再在该批次内查询角色待办；运营当前没有“待我处理”行时仍显示真实最新批次号与完成时间，不能把“无本人待办”伪装成“无成功批次”。
