...
- 行动清单和运营工作台使用“稳定任务 + 最近前序记录”两层表达：稳定任务标识长期经营责任，最近前序记录说明当前周从哪条历史状态续接。
- 当前稳定任务 ID、当前动作、当前状态和当前生命周期时间常驻展示；最近前序的来源批次、动作、状态、产生时间和执行时间放入每条 SPU 独立的披露控件，默认折叠，避免历史信息压过当前任务。
- 折叠态只显示“最近前序待办”和展开提示，不透出历史字段；展开后一次显示完整前序摘要。使用原生 `details/summary` 或等价的可访问披露模式，支持鼠标点击与键盘 Enter/Space，展开动作不得触发行跳转。
...
| SPU 515 跨周待办 | 稳定任务 `TASK-TOY-515`；首次产生 `2026-06-29 09:12`；本周关联 `2026-07-06 09:18`；经营状态待执行；执行时间 `—`；最近前序批次 `BATCH-20260531-01`、动作清仓、状态待执行、产生时间 `2026-06-29 09:12`、执行时间 `—` |
