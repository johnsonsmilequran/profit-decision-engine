...
> 实现同步（2026-08-04）：仓库已建立 `web/`、`backend/`、`deploy/`、根 `compose.yaml` 与 `Makefile`。首个纵向切片使用 PostgreSQL `role_mapping/user_session`、真实钉钉 OAuth 服务端适配器、HttpOnly 会话及 React `/login`、`/auth/recovery`；缺少部署密钥时保持默认拒绝，不提供本地账号或认证绕过。会话生命周期由真实 PostgreSQL 集成测试覆盖创建、认证、角色停用立即拒绝与撤销。Compose 内数据库使用 `5432`，默认映射本机 `55432`以支持 API/Worker 独立启动与真实持久化冒烟；PostgreSQL 18 持久卷按官方镜像的主版本目录结构挂载到 `/var/lib/postgresql`。工程入口以根 README 与 Makefile 为准，后续闭环继续沿用同一目录和事务边界。
