...
export function WorkbenchPage({ expectedRole }: { expectedRole: 'operations' | 'supervisor' }) {
  ...
  return <AppShell active="workbench"><section data-page-id={supervisor ? 'PAGE-F06-01' : 'PAGE-F05-01'}>
    ...
    {query.isPending ? <State title="正在加载工作台" body="正在按当前身份读取真实任务。" /> : query.isError ? <State title="工作台加载失败" body="未显示任何可能过期的经营数据。" action={<button className="button" onClick={() => query.refetch()}>重新加载</button>} /> : query.data ? <>
      ...
      <div className="workbench-lower"><div className="panel section-panel"><h2>{supervisor ? '异常与冲突' : '数据限制'}</h2>{supervisor?<p className="muted">当前有 {query.data.exception_count} 条动作变化需要逐条确认。并发冲突不会覆盖已成功事实。</p>:query.data.data_limitations.length===0?<p className="muted">本批次未发现数据限制。</p>:<><ul className="limitation-list">{query.data.data_limitations.map(item=><li key={`${item.field}-${item.status}`}><strong>{limitationField[item.field]??item.field}</strong><span>{limitationStatus[item.status]??item.status} · 影响 {item.count} 个 SPU</span></li>)}</ul><a className="table-link" href={`/batches/${query.data.latest_batch_id}`}>进入批次详情核对来源与影响</a></>}</div><div className="panel section-panel"><h2>流程责任</h2><p className="muted">固定规则生成 → 主管整体审核 → 责任运营分别执行经营与 OA 协同 → 记录结果与完成确认。</p></div></div>
    </> : null}
  </section></AppShell>
}
