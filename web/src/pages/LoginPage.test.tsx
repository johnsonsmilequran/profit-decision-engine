...
describe('LoginPage', () => {
  it('only exposes the real browser click entry for DingTalk authentication', async () => {
    ...
    expect(container.querySelector('.click-login-note svg')).toBeInTheDocument()
    expect(container.querySelector('.primary svg')).toBeInTheDocument()
    expect(container.querySelectorAll('.trust-item svg')).toHaveLength(3)
    expect(container.querySelector('.auth-footnote svg')).toBeInTheDocument()
  })
})
