...
| 钉钉统一身份认证与角色映射 | 已选定，接入可用性待实施验证 | 事业部负责人审批；运维/系统管理员配置 | 用户确认以钉钉为统一身份入口，三类角色变更由事业部负责人审批，运维/系统管理员按审批结果配置并维护 | 认证失败或无有效映射时拒绝进入业务页；不得降级为共享账号、账号密码或仅前端隐藏 |
...
2. **跨周任务状态**：每周决策快照独立保留，但同一 SPU/商品链接的执行任务使用稳定任务 ID 跨批次续接。新批次动作相同时不重建任务、不重置状态或重复审核；动作变化时追加待主管确认的新版本，通过后替换当前生效动作。当前周视图同时关联最近一条前序经营待办记录，并区分任务首次产生时间、本周关联时间、当前经营状态和经营动作执行时间。
3. **动作级执行状态**：建议通过后，经营动作分配给运营，补货/禁补动作分配给采购计划；两类动作分别记录执行状态和结果。
...
| 事业部负责人 | 间接 | 决定试点范围、资源投入与角色授权边界 | 对项目范围和资源投入拍板；审批三类业务角色变更 | 不作为 V0 日常操作角色，不直接配置账号或角色 | V0 MVP |
| 数据支持部门 | 间接 | 按确认口径提供可导入的经营表 | 每周一提供经营表并协助澄清字段、统计期间与业务截止日 | 不承担建议审核与经营动作执行 | V0 MVP |
| 运维人员 | 间接 | 保障模型网关、凭据和身份映射可用 | 配置、更新、轮换系统级部署密钥；按事业部负责人审批结果配置三类角色映射 | 密钥仅作用于本产品环境；不得自行审批业务角色 | V0 MVP |

### 2.2 多角色协作结论

本产品不是单角色分析工具。运营与运营主管需要完整经营信息完成执行和审核；采购计划只接收补货相关最小必要信息。建议由运营主管一次审核；若主管不采纳固定规则动作，可在任一动作开始执行前建立人工改判版本。改判只改变“生效动作”，原规则结论与冻结证据永久保留；清仓改为加投时必须同步重新确认库存动作，不得沿用禁补。每周新批次仍产生独立规则快照，但同一 SPU 同动作直接关联已有任务并保留其审核/执行状态；动作变化才追加新版本等待主管确认。当前待办必须显示最近一条前序经营待办记录的来源批次、动作、状态、产生时间和执行时间，避免用户只看到“延续上周”却无法还原关系。建议通过后，经营与库存生效动作分别由运营和采购计划执行并记录结果。

“观察”不是无人负责的标签：由运营启动持续观察；当现有证据支持时，可执行价格、内容或详情页优化，并记录动作和结果。没有评价、退款原因或广告明细时，不生成超出 SPU 汇总证据的具体优化项。

三类直接用户统一通过钉钉完成身份认证，每人使用自己的钉钉身份登录；运营、运营主管、采购计划角色变更由事业部负责人审批，运维/系统管理员按审批结果配置并维护。产品只消费认证结果与已审批配置的角色映射，不提供账号密码、注册、找回密码或业务侧角色管理后台；无有效认证或无有效映射时默认拒绝进入业务页。
...
- `ACTION_ITEM` 表示稳定任务下分角色执行的动作轨。最多包含一个经营主动作项和一个补货/禁补动作项；两者不能独立审核。
...
| `DecisionTaskLink.previous_link_id` | 最近前序待办记录指针，可空 | 只指向同一事业部、同一 SPU 且业务时间早于当前记录的最近一条关联；首次建任为空 |
...
| 最近前序待办 | 只读关联摘要 | 首次建任显示“无前序待办” | 显示最近前序来源批次、动作、状态、产生时间与执行时间并可进入关联详情 | 按同一事业部与 SPU/商品链接精确匹配；不得跨 SPU 关联 |
...
9. 经营待办以事业部与 SPU/商品链接精确锚定；存在历史经营待办时，当前行必须关联按业务时间倒序得到的最近一条前序待办记录，而不是任意一条历史记录。
...
- 经营状态筛选的候选值来自受控经营状态枚举，不得用采购动作或建议总览状态代替。

### FR-6（F06）

**功能名称**：审核与执行跟踪  
**实现用户故事**：US-F06-01、US-F06-02  
**对应需求**：REQ-F06-01、REQ-F06-02、REQ-F06-03、REQ-F06-04
...
11. 清仓改为加投时不得继续使用“禁止补货”；主管必须基于冻结库存与销量依据重新选择补货、不补货或不生成采购任务。
...
3. 运营和运营主管可查看完整经营信息；只有主管审核，运营执行经营动作并记录经营结果。
4. 采购计划只接收补货/禁补任务、SPU 身份、库存、销量、可售天数和任务状态；可回填本任务状态/结果，但不可查看完整利润、推广或售后信息。
...
- “经营状态”是经营动作轨的受控状态，不等同于建议审核状态、采购动作状态或建议总览聚合状态，可独立筛选和恢复。
...
| 钉钉统一身份认证 | 为每位直接用户提供独立身份，并承载三类业务角色映射来源 | 公司钉钉身份由运维/系统管理员管理 | 事业部负责人审批角色变更；运维/系统管理员按审批结果维护映射及接入配置 | 钉钉身份体系与本产品服务端认证配置；产品无业务侧角色后台 | 玩具事业部直接用户 | 按审批结果维护；实施阶段验证连通性 | 认证失败、登录态失效或无映射时默认拒绝并引导联系运维/系统管理员，不降级为共享账号或账号密码 | REQ-F07-01～REQ-F07-03；AC-F07-01～AC-F07-06；NFR-002、NFR-005 |
...
| 钉钉认证与三类操作角色映射能力 | 已选定，接入可用性待实施验证 | 事业部负责人审批；运维/系统管理员配置 | 用户确认钉钉统一身份入口及双重责任；需验证能记录稳定 actor_ref | 不可用或无映射时拒绝进入业务页，不得以共享账号、账号密码或前端隐藏开放真实数据 | REQ-F06-01～REQ-F07-03；AC-F06-01～AC-F07-06；NFR-002、NFR-005 |
...
当当前批次中的 SPU/商品链接存在历史经营待办时，系统应在当前经营待办中显示稳定任务 ID、首次产生时间、本周关联时间、当前经营状态和经营动作执行时间，并关联显示按业务时间倒序得到的最近一条前序待办记录，包括其来源批次、动作、状态、产生时间和执行时间；尚未执行时执行时间明确显示“—”，不得用更新时间替代。

### AC-49（AC-F05-10，关联 REQ-F05-05）

当授权用户在行动清单或运营待办视图选择经营状态时，系统应只显示当前经营动作状态匹配的 SPU 待办，并在刷新或从建议详情返回后恢复该筛选；经营状态筛选不得被建议审核状态、采购动作状态或建议总览状态替代。
...
当运营主管将“清仓”改判为“加投”时，系统不应静默沿用规则派生的“禁止补货”；必须要求主管根据冻结库存与销量依据明确选择补货、不补货或不生成采购任务，再使改判生效。

### AC-44（AC-F06-09，关联 REQ-F06-04）

当原清仓经营动作或其关联禁补动作已进入执行时，系统应禁止直接改判，显示已执行的责任人、时间和状态，并要求主管先终止原经营与采购任务；终止和后续改判分别追加事件，不覆盖已发生执行事实。
...
当钉钉认证失败、登录态失效，或运维/系统管理员未按审批结果配置运营、运营主管、采购计划中的有效角色时，系统应拒绝进入业务页且不返回任何经营数据，并提供重新发起钉钉认证或联系运维/系统管理员的恢复指引；不得自动赋予默认角色或降级为共享账号、账号密码登录。
...
| 用户关于身份认证的确认 | 钉钉是统一身份入口；三类角色变更由事业部负责人审批，运维/系统管理员按审批结果配置并维护 | B |
...
| 身份认证与角色映射 | 已确认 | 使用钉钉统一身份认证；事业部负责人审批三类角色变更，运维/系统管理员按审批结果配置；产品不增加账号密码或业务侧权限后台 |
...
| 最近前序待办 | 同一事业部与 SPU/商品链接下，业务时间早于当前记录且距离当前最近的一条经营待办关联记录 |
...
| NFR-005 | FR-1、FR-3、FR-5、FR-6、FR-8 | 批次、规则、关键值、稳定任务、最近前序关系、version、操作者、产生/关联/执行时间和状态历史保留 |
