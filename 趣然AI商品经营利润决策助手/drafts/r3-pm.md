...
- 处理：✅ 接受核心意见。现代 `AC-Fxx-xx` 继续以 `id-registry.md` 为唯一稳定 ID；未新增、未复用现代 ID。
- 特殊裁决：legacy `AC-N` 按阶段 5 统一裁决保留为展示别名，不作为正式引用来源；这部分不采用“全部删除 legacy ID”的建议。
- 修订位置：§10 导语及全部 39 个 AC 标题。

#### P0-02 · 生成日导致新品与期间不确定

- 处理：✅ 接受。新品与上一个完整自然月统一绑定批次业务截止日，不使用上传、生成或访问时间。
- 修订位置：§0.2、US-F02-01、US-F08-01、§10.0、AC-F01-01、AC-F03-01、AC-F08-01。

#### P0-03 · 销售或利润缺失被整行排除

- 处理：✅ 接受字段级降级原则。PM AC 明确只有无法建立唯一 SPU 身份的错误排除整行，其他字段仅停止依赖它的计算。
- 修订位置：US-F01-01、AC-F01-04、AC-F02-04、AC-F03-02～03。

#### P0-04 · 禁止补货缺少采购状态更新

- 处理：✅ 接受。采购计划可记录“已确认禁补”或等价动作结果，该结果进入动作与审计链，但不伪装为外部采购系统回执。
- 修订位置：§2.1、US-F06-02、AC-F06-04、AC-F08-03～04。

#### P0-05 · 建议唯一状态与双动作状态冲突

- 处理：✅ 接受。改为建议级一次审核、动作级分别执行、页面只读汇总三层状态。
- 修订位置：§0.3、US-F06-01～02、AC-F06-01～06、AC-F08-03～04。

#### P1-01 · 真实经营问题计数实体不唯一

- 处理：✅ 接受。按批次内去重 `(batch_id, spu_id)`/建议决策计数，双动作仍计 1；结构化兜底完整时计入。
- 修订位置：§8.1、§8.2。

#### P1-02 · 动作 AC 错把所有字段都设为前置

- 处理：✅ 接受。新品先依赖上架日期，非新品再依赖销售额；清仓/止损不依赖品退率，加投与观察按各自字段条件执行。
- 修订位置：AC-F01-04、AC-F02-04、AC-F03-01～03。

#### P1-03 · 结果记录责任不清

- 处理：✅ 接受并按统一裁决细分。运营记录加投、观察、止损、清仓及后续经营结果；采购计划记录补货或禁补结果。
- 修订位置：§2.1、US-F06-02、AC-F06-03～05。

#### P1-04 · 10—20 个 SPU 被工程稿写成硬阻断

- 处理：✅ 在 PM 稿明确否定硬阻断。该数字只作为首轮验收规模。
- 修订位置：§0.1、§8.2、§10.0、AC-F01-01。

#### P1-05 · 批次幂等语义不统一

- 处理：✅ 只定义同一导入请求的点击、网络或并发重试幂等；不以文件摘要发明跨批次业务身份，也不承诺更正批次。
- 修订位置：AC-F01-02、AC-F05-01、NFR-005、§11.3。

#### P1-06 · 同动作内排序无稳定兜底

- 处理：✅ 接受。未确认代理规则时用稳定 SPU ID 或批次内生成序号；AI 不改变固定动作层级。
- 修订位置：AC-F05-04。

#### P1-07 · 用户侧重新生成解释超出 V0

- 处理：✅ 接受。PM 稿不承诺普通用户重试按钮或解释版本界面，只保留 AI 失败不阻塞的结果。
- 修订位置：AC-F04-04、§11.3。

#### P1-08 · 清单与批次可能形成第二套身份

- 处理：✅ 接受。行动清单定义为批次建议与动作的唯一投影。
- 修订位置：AC-F05-01～02。
...
- 处理：✅ 接受，但遵守“不新增 AC ID”的统一裁决。AC-F01-03 专门承载批次期间错误；AC-F01-04 承载行字段错误及按依赖降级。
- 修订位置：AC-F01-03～04。

#### PM-P1-02 · 缺少批次幂等 AC

- 处理：✅ 接受，不新增 ID。AC-F01-02 承载同一导入请求幂等；AC-F05-01 保证同批次唯一清单。
- 修订位置：AC-F01-02、AC-F05-01、NFR-005。

#### PM-P1-03 · 分类和动作边界不明确

- 处理：✅ 接受。补全自然月边界、20,000/100,000 销售额边界、-20%/0%/5%/10%/15% 利润边界、1.5% 品退边界和 30 天库存边界。
- 修订位置：§10.0、AC-F03-01、AC-F03-03、AC-F03-06。

#### PM-P1-04 · 缺少并发审核与重复状态提交结果

- 处理：✅ 接受，不新增 ID。AC-F06-06 改为并发与网络重试的唯一可观察结果。
- 修订位置：AC-F06-06、NFR-005。

#### PM-P1-05 · 双动作和结果责任冲突

- 处理：✅ 接受。建议审核与动作执行分层；运营、采购分别记录本人动作结果，经营指标结果仍由运营记录。
- 修订位置：§0.3、§2、US-F06-01～02、AC-F06-01～06。

#### PM-P1-06 · NFR-003 没有可观察结果

- 处理：✅ 接受但不新增/挪用 AC ID。单列 NFR-003 可观察结果：无普通用户密钥入口，页面、接口与错误详情不返回部署密钥。
- 修订位置：§10.6。

#### PM-P1-07 · legacy ID 未登记

- 处理：⚠️ 部分接受。认可现代 ID 必须是唯一稳定来源；但按阶段 5 统一裁决保留 legacy ID 作为纯展示别名，并明确其不参与引用、排序或复用。
- 修订位置：§10 导语。

#### PM-P1-08 · 溯源缺少规则版本

- 处理：✅ 接受。补充批次业务截止日和规则版本，并要求历史固化值不被当前规则覆盖。
- 修订位置：US-F08-01、AC-F08-01～02、§8.2。

#### PM-S-01 · 首批固定日期与通用期间未分层

- 处理：✅ 接受。首批继续显示 2026-06-01 至 2026-06-30；通用行为以每批声明的完整自然月和业务截止日为准，不信任“上一周”等自然语言表头。
- 修订位置：§0.2、AC-F01-01、AC-F02-01～02。

#### ARCH-OPEN-01 · `Asia/Shanghai` 未获用户确认

- 处理：✅ 按统一裁决收敛为“实现默认时区假设”，不标为用户业务拍板，也不补造用户确认记录。
- 修订位置：§0.2、§1.6、§10.0、§11.3。

## ⚠️ 待合稿同步的章节边界

- 架构与工程章节必须沿用本稿现代 AC 的最终语义，不得保留另一套同名 AC。
- 架构状态模型需采用建议级审核与动作级执行分层；清仓/止损派生禁补不可被单独驳回。
- 工程页面需允许采购计划更新禁补结果，并允许运营执行观察动作。
- AI 解释分支与审核执行并行；等待或失败不能成为清单审核的前置条件。
- 10—20 个 SPU 只用于首轮验收，不得作为运行时字段校验失败。
