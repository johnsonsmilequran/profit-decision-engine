...
- 分析对象：`output/requirements.md` revision 2。
...
| 外部能力责任 | PASS | LiteLLM 的密钥责任已明确；钉钉认证与三角色映射由 IT 维护。 |
...
| 统一身份与业务角色权限 | 钉钉负责统一身份认证，IT 维护三类角色映射；产品按映射做服务端鉴权，无认证或无映射时默认拒绝。 | REQ-F07-01、REQ-F07-02、REQ-F07-03 |
...
- 三角色身份认证与映射能力已形成 committed 责任承诺：使用钉钉统一身份认证，由 IT 维护运营、运营主管、采购计划角色映射；实施阶段仍需验证接入连通性，失败时默认拒绝，不得降级为共享账号、账号密码或仅前端隐藏。
