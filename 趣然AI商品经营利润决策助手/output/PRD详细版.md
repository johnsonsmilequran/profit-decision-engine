...
若上线 30 天的真实记录显示无角色申请极少且没有可归因经营影响，则不启动 OA 集成、周期复核、导出或其他高级治理投资；不为维持价值假设而扩张功能。该停止线不虚构“极少”的数值阈值，由 30 天人工复盘基于真实记录判断。
...
| 运维人员 | 间接 | 保障模型网关、OA 通道、身份接入和恢复部署可用 | 配置、更新、轮换系统级部署密钥、OA 接口凭据及一次性恢复材料；业务角色由唯一 Admin 按事业部负责人审批结果配置 | 密钥仅作用于目标环境；不得审批或配置业务角色 | V0 MVP + F09 MVP |
...
| 指标 | 可取事实 | 可信口径 |
|---|---|---|
| 首次初始化授权闭环 | 同一 `DeploymentEnablement` 的里程碑 1→7 | §0.5 全部门禁满足后才有效；缺节点为“无法形成周期” |
...
- **敏感信息**：密码、setup/recovery 原始口令和完整会话令牌永不回显；TOTP 绑定二维码/手工密钥与新恢复码仅能在初始化或恢复的受保护绑定步骤中，向当前 Admin 持有人通过 `no-store` 响应当次展示。上述秘密均不得进入 URL、前端持久化/缓存、前端包、通用业务响应、日志、审计详情、镜像或仓库，离开步骤后不得重显。
...
| 恢复通知处置 | 安全设置受限列表/详情 | 显示未闭环任务、关联号、负责岗位与联系方式 | 查看重试结果，提交组织认可的补偿证据引用并由独立安全责任岗位确认闭环 | 非有效 Admin 会话；原始敏感证据不得上传 |
...
8. 部署恢复的职责不共用长期凭据：恢复执行人（运维）只配置一次性恢复材料并开启限时、限范围的受控交接；Admin 账号持有人本人在受保护绑定步骤中输入新密码（系统始终按密码字段处理且不回显），并仅当次看到 TOTP 绑定密钥和恢复码；恢复执行人和通知接收岗位只看到事件元数据，不得看到或代设长期凭据。物理人不同时交接能力一次使用、完成即失效，不产生共享密码。
9. 恢复页退出后，通知失败任务仍可从“安全设置 > 恢复通知处置”或对应审计事件详情返回。系统自动重试并显示状态；恢复执行人在产品外的受控工单产生不含原始敏感内容的补偿证据引用，恢复后的 Admin 只能在上述入口查看状态或代录该引用，独立安全责任岗位在产品外完成核验并回写确认后才进入 `compensated_closed`；新凭据登录不因通知待处置被锁死。
...
| 目标业务角色与工作台 | 只读结果 | 待处理时不显示 | 已配置待重新登录时显示唯一目标角色、对应工作台及旧会话已失效 | 其他状态 |
...
5. 配置成功后状态为“已配置待重新登录”，明确显示唯一目标角色、对应工作台和旧会话已失效；用户不能在此改选角色。仍打开的待处理页刷新或重新查询即可看到新状态，但不能热升级为业务会话。用户必须重新完成钉钉认证并进入正确工作台后，状态才显示“已授权”。
...
| 解除产品内双映射冲突 | 单人高风险操作 | 仅当冲突来自产品内多条当前映射时可用 | 基于新的有效审批证据和当前版本选定唯一最终角色，原子失效冲突映射、撤销旧会话并追加审计 | 身份源/企业作用域或组织属性冲突；证据或版本不完整 |
...
10. 配置冲突必须显示类型、负责岗位、有效联系方式、所需证据与刷新/重试入口。若仅为产品内重复/双角色映射，Admin 通过上述单人解除操作，以新的有效审批证据和当前映射版本选定最终唯一角色，原子失效其他映射、撤销受影响会话并追加审计；不合并两个身份。若为企业作用域/身份源冲突或组织属性缺失、矛盾，产品内不允许修改，只由运维/身份管理责任人在权威源纠正；纠正后 Admin 刷新重新投影为待处理、已授权、已停用或仍为冲突。
...
| 恢复通知处置 | 安全设置受限入口 | 仅显示未闭环任务 | 返回任务详情、重试状态和补偿证据引用 | 非 Admin；通知已闭环 |
...
8. 所有管理写接口必须同时通过 Admin 会话、CSRF token、严格 Origin 和预期 Content-Type 校验；任一失败均拒绝且不执行写入。运行时数据库身份对管理审计表无 UPDATE、DELETE 或 DDL 权限。
...
1. 首次初始化授权目标只在 §0.5 的十项生产上线门禁全部就绪时计量；§9.7.G 仅解释实现分层，不能替代 §0.5。计时从服务端初始化开始事件到目标用户用正确角色版本业务会话访问工作台，目标不超过 10 分钟。
...
| LiteLLM 网关与系统级部署密钥 | 已完成生产链路验证 | 运维 | 2026-08-05 真实 XLSX 经 API→Worker→LiteLLM→PostgreSQL 验证；继续维护凭据轮换和失败告警 | 不阻断规则清单、审核执行；解释显示失败 | REQ-F04-01～REQ-F04-03；AC-F04-01～AC-F04-04；NFR-003、NFR-004 |
...
| 钉钉企业内部机器人单聊 | 已完成真实投递与幂等验证 | 运维/系统管理员 | 2026-08-05 官方回执 SUCCESS、单收件人且同日 Worker 无重复；继续维护凭据轮换和失败告警 | 失败显示待补发并由责任运营人工钉钉协同；不改变业务确认状态 | REQ-F06-02、REQ-F06-05、REQ-F07-02；AC-F06-04、AC-F06-10～12、AC-F07-03～04 |
...
F09 第一版完成仍以 §0.5 的十项完整门禁清单为唯一总口径；AC-F09-15 的 10 分钟结果只有在该清单全部满足时才有效。§9.7.G 只解释实现分层，不替代 §0.5。
...
当系统尚不存在 Admin，且操作者提供当前环境有效的一次性 setup 口令并完成用户名、合规密码、TOTP 实码验证和恢复码保存确认时，系统应仅在受保护且 `no-store` 的绑定当次向本人展示 TOTP 密钥和新恢复码，只创建一名 Admin，消费该口令，关闭 setup 入口，追加初始化审计，并引导该 Admin 使用新凭据完成首次正式登录；离开绑定步骤后不得重显。
...
当 setup 口令无效、过期、已消费、环境不匹配，或初始化发生重复提交、并发提交、刷新、返回、网络中断或结果未知时，同一初始化请求应使用稳定幂等键，系统应给出不泄露精确口令状态的可操作结果；不得产生半成品或第二名 Admin。幂等重放必须返回可判定的同一已提交/未提交结果，不重复创建 Admin、消费材料或追加审计。
...
当正常凭据和保存型恢复码均不可用，且恢复执行人依据独立恢复审批提供当前环境有效的一次性受控交接，Admin 账号持有人本人完成新密码及新 TOTP 实码验证时，系统仅在受保护且 `no-store` 的绑定当次向 Admin 持有人展示新 TOTP 密钥和恢复码；恢复执行人与通知岗位不得看到长期凭据。只有新凭据和恢复码生效、凭据代次更新、全部旧 Admin 会话撤销、部署恢复口令消费、恢复审计和通知任务持久化同时成功，系统才显示“恢复已安全完成”；内部任一步失败时旧凭据、旧会话和恢复口令状态均不得部分切换。通知状态按 `pending_delivery` 待投递、`delivered` 已投递、`failed_pending_compensation` 失败待补偿、`compensated_closed` 补偿已闭环展示；恢复页退出后仍可从安全设置/审计详情返回任务，外部失败不回滚已完成恢复，也不得伪装已闭环。

### AC-F09-10（关联 REQ-F09-05）

当审批证据中的目标人员、企业作用域、稳定标识或可核验组织属性不能与当前目标身份唯一对应，两个可靠属性不足或互相冲突，审批证据缺失或不被组织认可，审批人不属于组织认可的审批责任范围，或目标角色不是运营/运营主管之一时，系统应拒绝授权，保留原角色和原会话状态，显示冲突类型、缺失项、精确负责岗位/联系方式、所需证据与刷新/重试入口，并追加不含敏感材料的拒绝审计。仅产品内重复/双映射可由 Admin 在新有效审批和当前版本下选定一个最终角色并原子解决；作用域、身份源或组织属性冲突只能由运维/身份管理责任人在权威源纠正后刷新投影。

### 10.10 F09 状态切换

### AC-F09-05（关联 REQ-F09-03）

当 Admin 已通过用户名和密码并提交一枚有效保存型恢复码时，系统应先要求绑定新 TOTP 并通过实码验证，仅在受保护且 `no-store` 的绑定当次向 Admin 持有人展示新 TOTP 密钥与新恢复码；只有验证成功后，旧恢复码消费、新 TOTP 与一枚新保存型恢复码生效、凭据代次更新、全部旧 Admin 会话撤销和管理审计才同时成立。任一步失败时，旧恢复码、旧 TOTP 和旧会话状态均不发生部分切换；保存型恢复码不得用于重设密码。稳定幂等键的重放返回同一已提交/未提交结果，不生成第二组凭据或审计。

### AC-F09-08（关联 REQ-F09-04）

当同一无角色用户重复完成钉钉认证时，系统应更新同一身份的最近认证时间而不覆盖首次待处理时间、不重复建单；Admin 首页应持续显示新申请和按首次进入当前待处理周期的等待时长可识别的最老待处理项。产品在角色提交成功前不得把该身份显示为“已审批”，配置成功后应显示已配置待重新登录、唯一目标角色、对应工作台和旧会话已失效，且用户不能改选角色；首次进入、停用或冲突后应分别显示已授权、已停用或配置冲突；已停用身份重复认证仍显示已停用。
...
当两个角色写操作基于同一旧状态提交时，系统应最多接受一个成功结果，后到操作显示最新状态且不新增第二条成功审批或审计；当钉钉登录与角色操作并发时，只有能证明绑定提交后最终唯一角色的业务会话才能进入工作台，旧角色或无法证明最终角色的会话不可继续使用并显示需重新登录；当稳定幂等键的同一请求因网络重放重复到达时，系统应返回可判定的同一已提交/未提交结果，不重复写入审批、映射、撤销或审计。任何分支均不得产生双有效角色。

### 10.11 F09 兼容与不变行为

### AC-F09-13（关联 REQ-F09-06）

当同一浏览器同时存在 Admin 登录态和钉钉业务登录态时，管理区只认 Admin 登录态，经营业务区只认运营或运营主管业务登录态；任一登录态缺失、失效或权限不足时对应区域独立拒绝访问，不回退、转换或共享身份。所有管理写接口必须同时通过 Admin 会话、CSRF token、严格 Origin 和预期 Content-Type 校验，任一失败均不执行写入。Admin 未另行以钉钉获得业务角色时不得读取任何经营数据；已有唯一有效业务角色的用户继续按 F01—F08 原入口进入原工作台，F09 角色操作只撤销受影响会话，不覆盖既有经营数据、任务、结果或业务审计。

### 10.12 F09 审计可观察结果

### AC-F09-14（关联 REQ-F09-06）

当发生 Admin 初始化及失败、登录限制、保存型恢复、部署恢复、角色新增/变更/停用/恢复、身份或审批拒绝、会话撤销和管理安全变化时，系统应追加可查询的管理审计；Admin 只能读取而不能修改或删除，运行时数据库身份对管理审计表无 UPDATE、DELETE 或 DDL 权限。审计不得展示密码、setup/recovery 口令、TOTP 密钥、恢复码或完整会话令牌；审计记录失败时对应高权限操作不得显示为成功。
...
| NFR-007 管理凭据与恢复材料保护 | 仅受保护绑定当次可向 Admin 持有人单次展示 TOTP 密钥与新恢复码；后续响应、存储、日志和审计不返回秘密；失效、恢复和轮换后旧材料不可继续使用 | AC-F09-01—02、AC-F09-04—06、AC-F09-14 |
...
| NFR-007 管理凭据与恢复材料保护 | FR-9.1—FR-9.3 | 只在受保护绑定当次单次展示 TOTP/新恢复码；不进后续响应、日志或前端存储；恢复单次消费，旧材料失效 |
