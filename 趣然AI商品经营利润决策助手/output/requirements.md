...
| 钉钉统一身份认证与双角色映射 | committed | 事业部负责人审批；运维/系统管理员配置 | 当前 Client 凭据已真实取得 Access Token 且 OAuth 授权跳转成功；最终回调仍需测试账号的稳定 `unionId` 命中事业部负责人已审批、运维已配置的唯一运营或运营主管映射；当前应用缺少单用户详情最小权限，不扩建全组织同步 | 不可用或无有效角色映射时拒绝进入业务页，不得降级为共享账号、账号密码或仅前端隐藏 | MVP | REQ-F07-01、REQ-F07-03 |
...
### REQ-F06-02 · 由责任运营在产品内闭环经营与库存动作

- Story: US-F06-02
- Stage: MVP
- Revision: 8
- Status: active
- Behavior: 管理该 SPU 的运营是产品内唯一执行责任人，经营动作与补货/禁补库存动作均直接在产品内确认执行；两轨状态独立但保持同一建议关联。库存动作必须填写真实处理说明后才能从“待执行”进入“已处理”，不发送钉钉机器人消息、不要求 User ID、不等待外部系统回执。
...
### REQ-F06-05 · 在产品内确认清仓完成时间

- Story: US-F06-02
- Stage: MVP
- Revision: 9
- Status: active
- Behavior: 清仓动作跨周保持不变时沿用原稳定任务；责任运营在产品内填报实际清仓完成时间并提交运营主管确认，主管确认后任务才闭环，退回则进入“完成时间待修正”。待确认任务通过工作台和行动清单持续可见，不创建钉钉催办、通知失败或人工补发记录。
...
THE SYSTEM SHALL 允许责任运营在产品内直接填写非空处理说明并确认库存动作已处理，保存动作状态、时间、操作者、备注和 version；页面和接口不得要求钉钉 User ID、发送钉钉通知或提供人工补发入口。
```
...
THE SYSTEM SHALL 记录确认人、确认时间和最终清仓完成时间，将清仓任务置为“已确认完成”；主管退回时必须填写原因，任务进入“完成时间待修正”且不丢失原提交记录。
```
...
THE SYSTEM SHALL 在责任运营工作台、主管工作台和行动清单中持续显示该任务的“待提交”“待主管确认”或“待修正”状态，并提供进入单条建议处理的入口；不得创建外部通知、每日催办或人工补发任务。
```
```
...
- Revision: 7
- Status: active
- Behavior: 系统使用钉钉作为统一身份认证入口，MVP 只由本机普通浏览器的点击操作发起 OAuth，不承诺扫码登录或钉钉客户端/工作台入口；系统读取经事业部负责人审批、由运维/系统管理员按审批结果配置的运营、运营主管角色映射；无有效认证或无有效角色映射时拒绝进入业务页，产品不提供采购、仓库等外部协同角色、账号密码、注册、找回密码或业务侧角色管理入口。
